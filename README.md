...
`pvdf` is made of three components: `pvscanner`, `vgsd`, and the `pvdf` command
...
For this, it will need the help of a service (`vgsd.service`) installed on the node itself, as a container is unable to perform directly LVM system calls on the host.
